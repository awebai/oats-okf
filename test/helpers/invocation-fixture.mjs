...
export function invocationFor({binding,context,action,subject=persistentSubject(),instance,priorReceipt=null,messagingChoice}={}) {
  ...
  return {schemaVersion:1,executionBinding:{schemaVersion:1,deployment:'/deployment/fixture',resolution:{schemaVersion:1,id:`sha256-${'b'.repeat(64)}`}},subject,
    instance:instance===undefined?{home:'/deployment/fixture/example',work:'/deployment/fixture/example/work',name:'example-1',agent:subject.kind==='persistent'?subject.soul.alias:subject.name}:instance,
    context,responsibleHuman:messaging.enabled?messaging.privateKey.human:null,messagingChoice:messaging,capability:binding.capability,action,priorReceipt};
}
